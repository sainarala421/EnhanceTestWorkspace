...
import co.nz.enhanceconsulting.utils.RunOnFailureKeywordsAdapter;
import co.nz.enhanceconsulting.utils.Selenium2LibraryFatalException;
import co.nz.enhanceconsulting.utils.Selenium2LibraryNonFatalException;
import co.nz.enhanceconsulting.locators.ElementFinder;
import co.nz.enhanceconsulting.locators.WindowManager;
...
public class BrowserManagement extends RunOnFailureKeywordsAdapter {
	...
	protected WebDriver createLocalWebDriver(String browserName, DesiredCapabilities desiredCapabilities) {
		if ("ff".equals(browserName) || "firefox".equals(browserName)) {
			...
			options.setCapability("firefox", desiredCapabilities);
			...
			options.setCapability("internetexplorer", desiredCapabilities);
			return new InternetExplorerDriver(options);
		} else if ("gc".equals(browserName) || "chrome".equals(browserName) || "googlechrome".equals(browserName)) {
			ChromeOptions options = new ChromeOptions();
			options.setCapability("chrome", desiredCapabilities);
			return new ChromeDriver(options);
		} else if ("opera".equals(browserName)) {
			// return new OperaDriver(desiredCapabilities);
			OperaOptions options = new OperaOptions();
			options.setCapability("opera", desiredCapabilities);
			return new OperaDriver(options);
		} else if ("phantomjs".equals(browserName)) {
			return new PhantomJSDriver(desiredCapabilities);
		} else if ("safari".equals(browserName)) {
			// return new SafariDriver(desiredCapabilities); -- FOR DELETION
			...
			try {
				return new AndroidDriver<WebElement>(new URL(""), desiredCapabilities);
			} catch (Exception e) {
	...
	protected void parseBrowserOptionsFirefox(String browserOptions, DesiredCapabilities desiredCapabilities) throws IOException {
	...
	protected void setRemoteWebDriverProxy(HttpCommandExecutor httpCommandExecutor) {
		...
		try {
			...
			// DefaultHttpClient clientx = (DefaultHttpClient) field.get(httpCommandExecutor);
			HttpClientBuilder client = (HttpClientBuilder) field.get(httpCommandExecutor);
			...
			if (remoteWebDriverProxyDomain.length() == 0) {
				...
				client.setDefaultCredentialsProvider(credentialsProvider).build();
			} else {
				// NTLM Authentication
